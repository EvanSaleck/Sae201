...
public class Accueil extends Stage {
	...
	private ObservableList<Arrivee> data = FXCollections.observableArrayList();
	...
	public Parent creerContenu(){
		...
		buttonPageArrivees.setOnAction(e -> {
			Stage stage = (Stage) buttonPageArrivees.getScene().getWindow(); // Récupère l'instance de Stage actuelle
		    Chambrepage accueil = null;
			try {
				accueil = new Chambrepage();
		...
		buttonPagePrestations.setOnAction(e -> {
			Stage stage = (Stage) buttonPagePrestations.getScene().getWindow(); // Récupère l'instance de Stage actuelle
		    Prestations accueil = new Prestations();
		    stage.setScene(accueil.getScene());
        });
		
		buttonPageChambres.setOnAction(e -> {
			Stage stage = (Stage) buttonPageChambres.getScene().getWindow(); // Récupère l'instance de Stage actuelle
		    Arrivepage accueil = null;
			try {
				accueil = new Arrivepage();
			} catch (FileNotFoundException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
	...
	public void addArrivee(ObservableList<Arrivee> data) {
	    ...
	    for (Arrivee arrivee : data) {
	        ...
	        if (arriveeDate.isEqual(now)) {
	        	Arrivees.getItems().add(arrivee);
	        } 
	    }
	    System.out.println(Arrivees.getItems().size());
